...
function testResolvers() {
  withEnv(
    {
      ...
      ALLOW_CUSTOMER_NOTIF_OVERRIDES: undefined,
  ...
  withEnv({ NODE_ENV: 'development', CUSTOMER_NOTIF_SMS_TO: '' }, () => {
  ...
  withEnv(
    {
      NODE_ENV: 'development',
      CUSTOMER_NOTIF_EMAIL_TO: 'mailto:admin@selorg.com',
    },
    () => {
      assert.strictEqual(resolveEmailTo('user@example.com'), 'admin@selorg.com');
    }
  );
  withEnv({ NODE_ENV: 'development', CUSTOMER_NOTIF_EMAIL_TO: '' }, () => {
    assert.strictEqual(
      resolveEmailTo('no-email-x@no-email.selorg', 'real@selorg.com'),
      'real@selorg.com'
    );
  });

  // Production must deliver to the real customer unless explicitly opted in.
  withEnv(
    {
      NODE_ENV: 'production',
      ALLOW_CUSTOMER_NOTIF_OVERRIDES: undefined,
    ...
    () => {
      assert.strictEqual(customerNotifOverridesAllowed(), false);
      assert.strictEqual(resolveEmailTo('user@example.com'), 'user@example.com');
      assert.strictEqual(resolveSmsTo('9876543210'), '9876543210');
      assert.strictEqual(resolveWhatsAppTo('9000000000'), '9000000000');
    }
  );
  ...
  withEnv(
    {
      NODE_ENV: 'production',
      ALLOW_CUSTOMER_NOTIF_OVERRIDES: 'true',
      CUSTOMER_NOTIF_EMAIL_TO: 'admin@selorg.com',
    },
    () => {
      assert.strictEqual(customerNotifOverridesAllowed(), true);
      assert.strictEqual(resolveEmailTo('user@example.com'), 'admin@selorg.com');
    }
  );
