...
const equipmentController = {
  getDevices: asyncHandler(async (req, res) => {
    const devices = await equipmentService.listDevices(req.user.warehouseKey);
    res.status(200).json({ success: true, data: devices, meta: { count: devices.length } });
  }),

  getDeviceDetails: asyncHandler(async (req, res) => {
    const device = await equipmentService.getDeviceById(req.user.warehouseKey, req.params.id);
    res.status(200).json({ success: true, data: device });
  }),

  getMachinery: asyncHandler(async (req, res) => {
    const machinery = await equipmentService.listMachinery(req.user.warehouseKey);
  ...
  getEquipmentDetails: asyncHandler(async (req, res) => {
    const equipment = await equipmentService.getEquipmentById(req.user.warehouseKey, req.params.id);
  ...
  exportEquipment: asyncHandler(async (req, res) => {
    const csv = await equipmentService.exportEquipment(req.user.warehouseKey);
