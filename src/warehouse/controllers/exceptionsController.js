...
const exceptionsController = {
  getExceptions: asyncHandler(async (req, res) => {
    const exceptions = await exceptionsService.listExceptions(req.user.warehouseKey, req.query);
    res.status(200).json({ success: true, count: exceptions.length, data: exceptions });
  }),

  reportException: asyncHandler(async (req, res) => {
    const exception = await exceptionsService.reportException(req.user.warehouseKey, req.body);
    res.status(201).json({ success: true, data: exception });
  }),

  getExceptionDetails: asyncHandler(async (req, res) => {
    const exception = await exceptionsService.getExceptionById(req.user.warehouseKey, req.params.id);
    res.status(200).json({ success: true, data: exception });
  }),

  updateStatus: asyncHandler(async (req, res) => {
    const exception = await exceptionsService.updateStatus(req.user.warehouseKey, req.params.id, req.body.status);
    res.status(200).json({ success: true, data: exception });
  }),

  rejectShipment: asyncHandler(async (req, res) => {
    const exception = await exceptionsService.handleShipmentRejection(req.user.warehouseKey, req.params.id);
    res.status(200).json({ success: true, message: 'Shipment rejected', data: exception });
  }),

  acceptPartial: asyncHandler(async (req, res) => {
    const exception = await exceptionsService.handlePartialAcceptance(req.user.warehouseKey, req.params.id, req.body.acceptedQuantity);
  ...
  exportExceptions: asyncHandler(async (req, res) => {
    const exceptions = await exceptionsService.listExceptions(req.user.warehouseKey);
    res.status(200).json({
      success: true,
