...
const interWarehouseController = {
  getTransfers: asyncHandler(async (req, res) => {
    const transfers = await interWarehouseService.listTransfers(req.user.warehouseKey);
    res.status(200).json({ success: true, count: transfers.length, data: transfers });
  }),

  requestTransfer: asyncHandler(async (req, res) => {
    const transfer = await interWarehouseService.requestTransfer(req.user.warehouseKey, req.body);
    res.status(201).json({ success: true, data: transfer });
  }),

  getTransferDetails: asyncHandler(async (req, res) => {
    const transfer = await interWarehouseService.getTransferById(req.user.warehouseKey, req.params.id);
    res.status(200).json({ success: true, data: transfer });
  }),

  updateStatus: asyncHandler(async (req, res) => {
    const transfer = await interWarehouseService.updateStatus(req.user.warehouseKey, req.params.id, req.body);
    res.status(200).json({ success: true, message: 'Status updated', data: transfer });
  }),

  getTracking: asyncHandler(async (req, res) => {
    const tracking = await interWarehouseService.getTracking(req.user.warehouseKey, req.params.id);
