...
var _paymentValidation = require("../../utils/paymentDetails.validation.js");
...
var preferredLocationSchema = _zod.z.object({
 ...
 // Rider home view – consolidated payload for rider app home screen
 riderRouter.get("/home", _authenticate.authenticate, /*#__PURE__*/function () {
   var _refHome = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _calleeHome(req, res) {
     var riderId, view, _tHome;
     return _regenerator().w(function (_contextHome) {
       while (1) switch (_contextHome.p = _contextHome.n) {
         ...
         case 3:
           _contextHome.p = 3;
           _tHome = _contextHome.v;
           if (_tHome && _tHome.statusCode === 404) {
             res.status(404).json({ error: _tHome.message || "Rider not found" });
           } else {
             res.status(500).json({ error: "Failed to build rider home view" });
           }
         case 4:
           return _contextHome.a(2);
       }
     }, _calleeHome, null, [[1, 3]]);
   }));
   return function (_xHomeReq, _xHomeRes) {
...
riderRouter.patch("/riders/:riderId", _authenticate.authenticate, /*#__PURE__*/function () {
  var _ref8 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee8(req, res) {
    var parseResult, rider, bankCheck, upiCheck, profileCheck, _t8;
    return _regenerator().w(function (_context8) {
      while (1) switch (_context8.p = _context8.n) {
        ...
        case 1:
          if (parseResult.data.bankDetails) {
            bankCheck = _paymentValidation.validateBankDetails(parseResult.data.bankDetails);
            if (!bankCheck.valid) {
              res.status(400).json({
                success: false,
                message: "Invalid bank account details",
                errors: _paymentValidation.prefixErrors("bankDetails", bankCheck.errors)
              });
              return _context8.a(2);
            }
            parseResult.data.bankDetails = Object.assign({}, parseResult.data.bankDetails, bankCheck.normalized);
          }
          if (parseResult.data.upiDetails) {
            upiCheck = _paymentValidation.validateUpiDetails(parseResult.data.upiDetails);
            if (!upiCheck.valid) {
              res.status(400).json({
                success: false,
                message: "Invalid UPI details",
                errors: _paymentValidation.prefixErrors("upiDetails", upiCheck.errors)
              });
              return _context8.a(2);
            }
          ...
          if (parseResult.data.name !== undefined || parseResult.data.email !== undefined || parseResult.data.phoneNumber !== undefined) {
            ...
            if (profileCheck.normalized.name !== undefined) {
              parseResult.data.name = profileCheck.normalized.name;
            }
