...
const { sendSms } = require('../../customer-backend/services/otpProviderService');

function generateCustomerTempPassword() {
...
function buildPasswordInfo(customer) {
  return {
...
const getPasswordInfo = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      data: buildPasswordInfo(customer),
    });
  } catch (error) {
...
const resetPassword = async (req, res, next) => {
  try {
    const { id } = req.params;
    const sendSmsNotification = req.body?.sendSms !== false;
    ...
    const plainPassword = generateCustomerTempPassword();
    ...
    if (sendSmsNotification && customer.phoneNumber) {
      try {
        const digits = String(customer.phoneNumber).replace(/\D/g, '').slice(-10);
        if (digits.length === 10) {
          const smsResult = await sendSms({
            to: digits,
            text: `Your Selorg account password was reset. Temporary password: ${plainPassword}. Sign in and change it when possible.`,
          });
          smsSent = Boolean(smsResult?.success);
        }
      } catch (smsErr) {
        logger.warn('Customer password reset SMS failed', { err: smsErr.message, customerId: id });
      }
    }
    ...
    res.json({
      ...
      data: {
        ...
        smsSent,
        passwordInfo: buildPasswordInfo(customer),
      },
    });
  } catch (error) {
