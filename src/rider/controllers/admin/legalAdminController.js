...
exports.getDocument = async (req, res) => {
  try {
    ...
    if (!doc || !isRiderLegalDoc(doc)) return res.status(404).json({ success: false, error: 'Document not found' });
...
exports.updateDocument = async (req, res) => {
  try {
    ...
    if (!existingCheck || !isRiderLegalDoc(existingCheck)) {
    ...
    if (body.isCurrent) {
      ...
      if (existing && isRiderLegalDoc(existing)) {
