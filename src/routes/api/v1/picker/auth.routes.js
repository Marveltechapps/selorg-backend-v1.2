/**
 * Picker Authentication Routes — Phase A API Standardization
 * File: src/routes/api/v1/picker/auth.routes.js
 *
 * P2.1: Picker service authentication endpoints
 * 
...
/**
 * POST /api/v1/picker/auth/register
 * Register new picker
 */
router.post('/register', async (req, res, next) => {
  try {
    ...
    const newPicker = {
      userId: 'picker_new_123',
      email,
      name
    };

    res.status(201).json(ResponseFormatter.success(newPicker, 'Picker registered successfully'));
  } catch (err) {
    next(err);
  }
});
...
/**
 * POST /api/v1/picker/auth/logout
 * Logout (protected endpoint)
 */
router.post('/logout', authenticateJWT, requireRole('PICKER'), async (req, res, next) => {
  try {
...
/**
 * GET /api/v1/picker/auth/profile
 * Get current user profile (protected)
 */
router.get('/profile', authenticateJWT, requireRole('PICKER'), async (req, res, next) => {
  try {
    const profile = {
