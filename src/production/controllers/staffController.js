...
const createShiftCoverage = async (req, res) => {
  try {
    const storeId = req.query.storeId || req.body.storeId || process.env.DEFAULT_STORE_ID || 'PROD-001';
    const { name, timeRange, startTime, endTime, requiredEmployees, date, department } = req.body;
    if (!name || !date) {
      return res.status(400).json({ success: false, error: 'name and date are required' });
    }
    const shiftKey = (name || '').toLowerCase().includes('morning') ? 'morning' : (name || '').toLowerCase().includes('afternoon') ? 'afternoon' : 'night';
    const shiftLabel = timeRange || name || `${startTime || ''}-${endTime || ''}`;
    const dateObj = new Date(date);
    const existing = await ShiftCoverage.findOne({
      store_id: storeId,
      date: { $gte: new Date(dateObj.toISOString().split('T')[0]), $lt: new Date(dateObj.toISOString().split('T')[0] + 'T23:59:59.999Z') },
      shift: shiftKey,
    });
    if (existing) {
      existing.target_staff = parseInt(requiredEmployees, 10) || existing.target_staff;
      existing.shift_label = shiftLabel;
      await existing.save();
      return res.status(200).json({ success: true, coverage: existing });
    }
    const coverage = new ShiftCoverage({
      shift: shiftKey,
      shift_label: shiftLabel,
      current_staff: 0,
      target_staff: parseInt(requiredEmployees, 10) || 20,
      status: 'scheduled',
      date: dateObj,
      store_id: storeId,
    });
...
const createStaff = async (req, res) => {
  try {
    ...
    const { name, staff_id, role, department, zone, current_shift } = req.body;
    if (!name || !staff_id || !role) {
      return res.status(400).json({ success: false, error: 'name, staff_id, and role are required' });
    }
    const existing = await Staff.findOne({ staff_id, store_id: storeId });
    if (existing) {
    ...
    const staff = new Staff({
      ...
      zone: zone || undefined,
      current_shift: current_shift || undefined,
      status: 'Offline',
      store_id: storeId,
    });
    await staff.save();
...
const updateStaffStatus = async (req, res) => {
  try {
    ...
    const storeId = req.query.storeId || process.env.DEFAULT_STORE_ID || 'PROD-001';
    const validStatuses = ['Active', 'Break', 'Offline', 'Meeting'];
    if (!status || !validStatuses.includes(status)) {
      return res.status(400).json({ success: false, error: 'Valid status required: Active, Break, Offline, Meeting' });
    }
...
const getAttendance = async (req, res) => {
  try {
    ...
    const records = await Attendance.find({ store_id: storeId, date }).lean();
    const withNames = await Promise.all(
      records.map(async (r) => {
        ...
        return {
          ...
          staff_id: r.staff_id,
          employee_name: staff?.name || 'Unknown',
          date: r.date,
          check_in: r.check_in,
...
const markAttendancePresent = async (req, res) => {
  try {
    ...
    const record = await Attendance.findOneAndUpdate(
      { record_id: recordId, store_id: storeId },
      {
        status: 'present',
    ...
    await Staff.findOneAndUpdate(
      { staff_id: record.staff_id, store_id: storeId },
      { status: 'Active' }
    );

    const staff = await Staff.findOne({ staff_id: record.staff_id }).lean();
    res.json({
      success: true,
      record: {
