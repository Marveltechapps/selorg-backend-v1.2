...
const createQCInspection = async (req, res) => {
  try {
    ...
    let status, score, items, defects, batch_id, product_name;
    if (batch && checkType && (result === 'pass' || result === 'fail' || result === 'pending')) {
      batch_id = batch;
      product_name = checkType;
      status = result === 'pass' ? 'passed' : result === 'fail' ? 'failed' : 'pending';
      items = 1;
      defects = result === 'fail' ? 1 : 0;
      score = result === 'pass' ? 100 : result === 'fail' ? 0 : 50;
    } else {
      items = parseInt(itemsInspected) || 0;
      defects = parseInt(defectsFound) || 0;
      score = items > 0 ? Math.round(((items - defects) / items) * 100) : 0;
      status = score >= 80 ? 'passed' : 'failed';
      batch_id = batchId;
      product_name = productName;
    }

    let factory_id = factoryId || null;
    if (lineId) {
      const line = await ProductionLine.findOne({ line_id: lineId }).lean();
      if (line) factory_id = line.factory_id;
    }
    ...
    const inspection = new QCInspection({
      ...
      check_type: checkType || product_name,
    ...
    // Update ProductionLine.defect_rate when lineId is provided
    if (lineId && items > 0) {
      const defectRate = (defects / items);
      await ProductionLine.findOneAndUpdate(
        { line_id: lineId },
        { defect_rate: defectRate, updated_at: new Date() }
      );
    }
...
const createSampleTest = async (req, res) => {
  try {
    ...
    const sample = new SampleTest({
      sample_id: `LAB-23-${Math.floor(Math.random() * 90000) + 10000}`,
      batch_id: batchIdVal || productNameVal,
      product_name: productNameVal,
      test_type: testType,
      source: source || batchIdVal,
